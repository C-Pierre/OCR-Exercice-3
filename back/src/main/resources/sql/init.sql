...
-- PARTICIPANTS
INSERT IGNORE INTO test.participate(session_id, user_id)
SELECT s.id, u.id
FROM test.sessions s, test.users u
WHERE u.last_name='Admin' AND s.name='Morning Yoga';

INSERT IGNORE INTO test.participate(session_id, user_id)
SELECT s.id, u.id
FROM test.sessions s, test.users u
WHERE u.last_name='Doe' AND s.name IN ('Morning Yoga', 'Power Stretch');
